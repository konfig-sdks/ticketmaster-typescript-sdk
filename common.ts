...
import { readableStreamToString, TicketmasterError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TicketmasterError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TicketmasterError(e, e.response?.data)
                }
                if (innerError instanceof TicketmasterError) {
                    // Got "TicketmasterError" from the above try block
                    throw innerError;
                }
